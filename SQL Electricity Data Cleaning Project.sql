CREATE DATABASE Energy_Consumption_Report;
...
ALTER TABLE `Energy_Consumption_Report`.`electricity_data_sql_project` 
RENAME TO  `Energy_Consumption_Report`.`electricity_data_2024` ;
...
-- We have now identified 16 duplicate values
...
CREATE TABLE `electricity_data_2024_clean2` (
  `Date` text,
...
INSERT INTO electricity_data_2024_clean2
SELECT *, 
ROW_NUMBER() OVER(PARTITION BY `Date`, `Value`, Unit) AS row_num 
FROM electricity_data_2024_clean;
...
DELETE FROM electricity_data_2024_clean2 
WHERE row_num > 1;
...
-- Date column was a string, so we need to change to DATETIME format

UPDATE electricity_data_2024_clean2
SET Date = str_to_date(Date, '%d/%m/%Y %H:%i')
WHERE str_to_date(Date, '%d/%m/%Y %H:%i') is not null;
